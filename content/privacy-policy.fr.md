---
title: "Politique de confidentialité"
slug: "politique-de-confidentialite"
seoTitle: "Politique de confidentialité | datanalyze"
description: "Ce site ne dépose aucun cookie et n'affiche aucun bandeau de consentement. Quelles données sont traitées, pourquoi, combien de temps et quels sont vos droits."
eyebrow: "Vie privée"
lead: "Ce site ne dépose aucun cookie et n'a donc besoin d'aucun bandeau de consentement. Voici, en clair, les données traitées et ce qu'il en advient."
---

## En résumé

{{% no-analytics %}}
- Le site **ne dépose aucun cookie** et **ne charge aucun script tiers**.
- **Aucune mesure d'audience** n'est en place : ni Google Analytics, ni équivalent.
- **Aucune ressource externe** n'est chargée : les polices, les icônes et les images sont hébergées sur ce site.
- Les seules données personnelles traitées sont **celles que vous transmettez volontairement** via le formulaire de contact ou la prise de rendez-vous.
{{% /no-analytics %}}
{{% analytics-notice %}}
- Le site **ne dépose aucun cookie**.
- La mesure d'audience se limite à **Cloudflare Web Analytics**, sans cookie, sans stockage local et sans donnée permettant de vous identifier.
- **Aucune autre ressource externe** n'est chargée : les polices, les icônes et les images sont hébergées sur ce site.
- Les seules données personnelles traitées sont **celles que vous transmettez volontairement** via le formulaire de contact ou la prise de rendez-vous.
{{% /analytics-notice %}}

C'est aussi la raison pour laquelle vous ne voyez aucun bandeau de consentement : il n'y a rien à consentir.

## Responsable du traitement

- **Responsable** : {{< legal "name" >}}, exerçant sous le nom {{< legal "tradingName" >}}
- **Adresse** : {{< legal "address.street" >}}, {{< legal "address.postalCode" >}} {{< legal "address.city" >}}, {{< legal "address.country" >}}
- **Contact** : {{< legal "email" >}}
- **Numéro d'entreprise (BCE)** : {{< legal "enterpriseNumber" >}}

## Cookies et traceurs

**Ce site n'utilise aucun cookie.** Il ne dépose rien sur votre appareil : ni cookie, ni stockage local, ni identifiant de session, ni pixel de suivi.

{{% no-analytics %}}
Il ne fait pas non plus appel à des ressources externes qui pourraient vous pister à votre insu : il n'y a ni police Google Fonts chargée depuis un CDN, ni bouton de réseau social, ni carte, ni vidéo intégrée.
{{% /no-analytics %}}
{{% analytics-notice %}}
La seule ressource externe chargée est le script de mesure d'audience décrit plus bas, qui ne dépose rien sur votre appareil. Pour le reste, il n'y a ni police Google Fonts chargée depuis un CDN, ni bouton de réseau social, ni carte, ni vidéo intégrée.
{{% /analytics-notice %}}
Le formulaire de contact et la prise de rendez-vous sont de **simples liens sortants** — précisément pour éviter que des cookies tiers ne soient déposés pendant votre visite.

Aucune page dédiée aux cookies n'existe donc sur ce site : il n'y aurait rien à y écrire.

## Mesure d'audience

{{% no-analytics %}}
**Aucun outil de mesure d'audience n'est installé.** Ni Google Analytics, ni Plausible, ni Matomo, ni aucun autre. Je ne sais donc pas combien de personnes visitent ce site, ni quelles pages elles consultent.
{{% /no-analytics %}}
{{% analytics-notice %}}
Ce site utilise **Cloudflare Web Analytics** pour mesurer la fréquentation de manière agrégée. Cet outil ne dépose aucun cookie, n'utilise aucun stockage local et ne collecte aucune donnée permettant de vous identifier ou de vous suivre d'un site à l'autre. Aucune donnée personnelle n'est traitée à ce titre, et aucun consentement n'est requis. Voir la [politique de confidentialité de Cloudflare](https://www.cloudflare.com/privacypolicy/).
{{% /analytics-notice %}}

Je dispose par ailleurs des informations suivantes :

- les **statistiques agrégées de Google Search Console** et de **Bing Webmaster Tools**, qui indiquent sur quelles requêtes le site apparaît dans les résultats de recherche — ces outils ne déposent rien sur votre appareil et ne me transmettent aucune donnée individuelle ;
- les **journaux techniques de l'hébergeur** (Netlify), conservés brièvement pour des raisons de sécurité et de fonctionnement, et auxquels je n'ai accès que de manière agrégée.

## Données que vous me transmettez

### Formulaire de contact

Si vous utilisez le [formulaire de contact](/contact/), vous êtes redirigé vers un formulaire hébergé par **Airtable**. Les données que vous y saisissez (nom, adresse e-mail, description de votre besoin, et toute autre information que vous choisissez d'ajouter) me sont transmises afin que je puisse répondre à votre demande.

- **Finalité** : répondre à votre demande et, le cas échéant, préparer une proposition.
- **Base légale** : votre demande elle-même — mesures précontractuelles prises à votre initiative (article 6.1.b du RGPD).
- **Destinataire** : moi seul. Vos données ne sont ni revendues, ni partagées, ni utilisées à des fins de prospection.
- **Conservation** : le temps de traiter votre demande, puis pendant la durée de la relation commerciale si elle se noue, et au maximum **trois ans** après notre dernier contact.

### Prise de rendez-vous

Si vous réservez un entretien, vous êtes redirigé vers **Calendly**, où vous communiquez votre nom, votre adresse e-mail et le créneau souhaité.

- **Finalité** : organiser l'entretien.
- **Base légale** : mesures précontractuelles prises à votre initiative.
- **Conservation** : identique à celle du formulaire de contact.

### Données confiées dans le cadre d'une mission

Si nous travaillons ensemble, vous serez amené à me transmettre des données à analyser. Elles relèvent d'un régime distinct :

- j'agis en qualité de **sous-traitant** au sens du RGPD, vous restant le responsable du traitement ;
- ces données ne sont **utilisées que pour la mission convenue**, et pour rien d'autre ;
- elles ne quittent pas mon poste de travail et ne sont transmises à aucun tiers ;
- elles sont **supprimées à votre demande** à la fin de la mission ;
- un **accord de confidentialité** peut être signé avant tout envoi ;
- pour des données de santé ou d'autres données sensibles, un **accord de sous-traitance** au sens de l'article 28 du RGPD est établi, et la transmission de données pseudonymisées est privilégiée.

## Sous-traitants et transferts

| Sous-traitant | Rôle | Politique |
|---|---|---|
| Netlify, Inc. | Hébergement du site | [Voir](https://www.netlify.com/privacy/) |
| Airtable, Inc. | Formulaire de contact | [Voir](https://www.airtable.com/company/privacy) |
| Calendly, LLC | Prise de rendez-vous | [Voir](https://calendly.com/legal/privacy-notice) |

Ces trois prestataires sont établis aux **États-Unis**. Les transferts de données s'effectuent sur la base des mécanismes prévus au chapitre V du RGPD (clauses contractuelles types et, le cas échéant, EU-U.S. Data Privacy Framework).

Vous n'êtes concerné par Airtable ou Calendly **que si vous cliquez volontairement** sur les liens correspondants. La simple consultation du site n'entraîne aucun transfert.

## Vos droits

Conformément au RGPD, vous disposez des droits d'**accès**, de **rectification**, d'**effacement**, de **limitation**, d'**opposition** et de **portabilité** sur vos données personnelles.

Pour les exercer, écrivez à {{< legal "email" >}}. Je réponds dans un délai d'un mois.

Vous pouvez également introduire une réclamation auprès de l'**Autorité de protection des données** (APD), rue de la Presse 35, 1000 Bruxelles — [autoriteprotectiondonnees.be](https://www.autoriteprotectiondonnees.be/).

## Sécurité

Le site est servi exclusivement en **HTTPS**. Les données de mission sont conservées sur un poste chiffré et ne sont pas déposées sur des services de stockage grand public.

## Modifications

Cette politique peut être mise à jour, notamment si un outil de mesure d'audience venait à être activé — auquel cas la section correspondante ci-dessus apparaîtra automatiquement.
